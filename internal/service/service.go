...
type Service struct {
	...
	webhook  gwook.WebhookNotifier
}

func NewService(settings *settings.Settings, webhook gwook.WebhookNotifier) *Service {
	return &Service{
		settings: settings,
		webhook:  webhook,
	}
...
func (svc *Service) startHttpServer() (*http.Server, context.CancelFunc) {
	...
	server := http.Server{
		...
		Handler:      NewRouter(svc.settings, svc.webhook),
